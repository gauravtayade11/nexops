...
export default function ReleaseManager() {
  ...
  const statusColors: Record<ReleaseStatus, string> = {
    ...
    deploying: 'bg-primary-50 text-primary-600',
    deployed: 'bg-success-50 text-success-600',
    rolled_back: 'bg-warning-50 text-warning-600',
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <h3 className="font-semibold text-gray-900 text-lg">v{release.version}</h3>
                      ...
                      {release.created_by && (
                        <p className="text-sm text-gray-500">Created by: {release.created_by}</p>
                      )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Create New Release</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900">Release v{release.version}</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <dl className="grid grid-cols-2 gap-2 text-sm">
              ...
              <dd className="font-medium capitalize">{release.status.replace('_', ' ')}</dd>
              <dt className="text-gray-500">Branch</dt>
              <dd className="font-medium">{release.release_branch}</dd>
              <dt className="text-gray-500">Source</dt>
              <dd className="font-medium">{release.source_branch}</dd>
              <dt className="text-gray-500">Target</dt>
              <dd className="font-medium">{release.target_branch}</dd>
            </dl>
          </div>

          {release.changelog && (
            <div>
              <h3 className="font-medium text-gray-900 mb-2">Changelog</h3>
          ...
          {release.commits.length > 0 && (
            <div>
              <h3 className="font-medium text-gray-900 mb-2">Commits ({release.commits.length})</h3>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {release.commits.map((commit, i) => (
                  <div key={i} className="p-2 bg-gray-50 rounded text-sm">
                    <code className="text-primary-600">{commit.sha}</code>
                    <span className="mx-2">-</span>
                    <span>{commit.message}</span>
                    <span className="text-gray-500 ml-2">by {commit.author}</span>
                  </div>
                ))}
              </div>
            </div>
...
function DeployModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Deploy v{release.version}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
