...
export default function SelfServicePortal() {
  ...
  const quickActions = [
    { type: 'scale' as ActionType, icon: ArrowsUpDownIcon, label: 'Scale', color: 'text-primary-600 bg-primary-50' },
    { type: 'restart' as ActionType, icon: ArrowPathIcon, label: 'Restart', color: 'text-warning-600 bg-warning-50' },
    { type: 'rollback' as ActionType, icon: ArrowUturnLeftIcon, label: 'Rollback', color: 'text-danger-600 bg-danger-50' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Developer Self-Service</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card">
          ...
          {actions.length === 0 ? (
            <div className="text-center py-8 text-gray-500">No actions yet</div>
          ) : (
            <div className="space-y-3">
              {actions.slice(0, 10).map((action) => (
                <div key={action.id} className="p-3 border border-gray-100 rounded-lg">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-medium text-gray-900 capitalize">
                        {action.action_type.replace('_', ' ')}
                      </p>
                      <p className="text-sm text-gray-500">{action.target_service}</p>
                    </div>
